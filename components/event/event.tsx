...
import { Bookmark, Calendar, Clock, Pencil, Share2, UserRound } from "lucide-react";
...
export const Event = <T extends ElementType = "li">({ as, callToActionButton, isEdit = false, className = "", ...props }: EventProps<T>) => {
  const Component = as || "li";

  return (
    <Component className={`bg-white pb-[18px] rounded-[5px] w-full text-[#1C1939] ${className}`} {...props}>
      <article className="flex gap-[30px] flex-col">
        <header>
          <div className="relative w-full h-[200px] sm:h-[187px]">
            <Image alt="Fitness Event Image" fill src="/images/fitness-event.png" />
          </div>
        ...
        <footer className="w-full px-[18px] justify-between gap-6 items-center flex flex-row">
          <Button className="font-semibold text-sm text-off-white ">Get Ticket for ₦1500</Button>
